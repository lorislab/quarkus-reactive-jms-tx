...
class JmsTxSource {
    ...
    JmsTxSource(JMSContext context, JmsTxConnectorIncomingConfiguration config, Jsonb json, Executor executor) {
        ...
        if (!broadcast) {
            source = ReactiveStreams.fromPublisher(publisher).map(m -> new IncomingJmsTxMessage<>(context, m, executor, json));
        } else {
            source = ReactiveStreams.fromPublisher(
                    Multi.createFrom().publisher(publisher)
                            .map(m -> new IncomingJmsTxMessage<>(context, m, executor, json))
    ...
    private Destination getDestination(JMSContext context, String name, JmsTxConnectorIncomingConfiguration config) {
    ...
    PublisherBuilder<IncomingJmsTxMessage<?>> getSource() {
        return source;
    }
